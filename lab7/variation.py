...
def _wdist(a,b):
	...
	d = Counter(b)
	diff = c - d
	numElements = len(list(diff.elements()))
	if numElements == 1:
		return diff.items()[0][1] 
	return numElements
...
def words_graph():
    ...
    try:
        try:
            import gzip
            fh=gzip.open('words_dat.txt.g','r')
        except:
    ...
    G = Graph(name="words")
    sys.stderr.write("Loading word4.dat: ")
...
if __name__ == '__main__':
    ...
    sp=shortest_path(G, 'cold', 'warm')
    print "shortest path between 'cold' and 'warm' is:\n", sp

    sp=shortest_path(G, 'love', 'hate')
    print "shortest path between 'love' and 'hate' is:\n", sp
